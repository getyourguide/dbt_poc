...
summary AS (
   SELECT
     ...
     , SUM(cm.realtime_ad_spend) OVER w_last_7_days  AS realtime_ad_spend_last_7_days
     , SUM(cm.realtime_ad_spend) OVER w_last_14_days AS realtime_ad_spend_last_14_days
     , SUM(cm.realtime_ad_spend) OVER w_qtd          AS realtime_ad_spend_qtd 
   FROM cm
   LEFT JOIN dates ON cm.date = dates.date_id

   WINDOW w_last_7_days AS (ORDER BY cm.date RANGE BETWEEN 6 PRECEDING AND CURRENT ROW)
   , w_last_14_days AS (ORDER BY cm.date RANGE BETWEEN 13 PRECEDING AND CURRENT ROW)
   , w_qtd AS (PARTITION BY quarter ORDER BY cm.date)
 )
